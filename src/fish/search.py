"""Corpus search — PRISM / legacy ANN with Qdrant metadata filters (no hybrid ranking)."""
...
def search_corpus(
    ...
    *,
    ...
    Metadata filters are applied inside Qdrant (before top-k). Superseded
    memories are dropped after fetch (rare).
    """
    ...
    with db_conn() as db:
        ...
        # Slight over-fetch so superseded memories don't shrink the page
        fetch_k = limit + 5
